    ...
    const result = {
      ...
      fd_prestat_dir_name: (memory, fd11, pathPtr, pathLen) => {
        try {
          ...
          const dirname2 = $preStatDirnames.get(fileDescriptor.fd);
          if (dirname2 === void 0) {
            return Promise.resolve(Errno.badf);
          }
          const bytes = $encoder.encode(dirname2);
...
((DeviceWasiService2) => {
  function create8(deviceDrivers, fileDescriptors, clock, virtualRootFileSystem, options) {
    ...
    const result = {
      ...
      path_open: async (memory, fd11, dirflags, path_ptr, path_len, oflags8, fs_rights_base, fs_rights_inheriting, fdflags11, fd_ptr) => {
        try {
          ...
          const result2 = await deviceDriver.path_open(fileDescriptor, dirflags, path, oflags8, fs_rights_base, fs_rights_inheriting, fdflags11, fileDescriptors);
    ...
    function getDeviceDriverWithPath(fileDescriptor, path) {
      const result2 = deviceDrivers.get(fileDescriptor.deviceId);
      if (!$path.isAbsolute(path) && virtualRootFileSystem !== void 0 && virtualRootFileSystem !== result2 && FileSystemDeviceDriver.is(result2)) {
        path = $path.normalize(path);
        if (path.startsWith("..")) {
          const virtualPath = virtualRootFileSystem.makeVirtualPath(result2, path);
      ...
      return [result2, fileDescriptor, path];
    }
...
function create(deviceId) {
  let inodeCounter = 0n;
  const decoder2 = ral_default().TextDecoder.create();
  ...
  const deviceDriver = {
    ...
    fd_write(_fileDescriptor, buffers) {
      ...
      ral_default().console.log(decoder2.decode(buffer));
...
var _FileSystem = class _FileSystem {
  ...
  existsNode(parent, path) {
    return this.getNodeByPath(parent, path) !== void 0;
  }
...
function create2(deviceId, baseUri, readOnly = false) {
  ...
  function createFileDescriptor(parentDescriptor, fd11, rights_base, fdflags11, path) {
    const parentNode = fs.getNode(parentDescriptor.inode, 1 /* Directory */);
    return new FileFileDescriptor(deviceId, fd11, rights_base, fdflags11, fs.getOrCreateNode(parentNode, path, 0 /* File */, true).inode);
  }
  ...
  function createDirectoryDescriptor(parentDescriptor, fd11, rights_base, rights_inheriting, fdflags11, path) {
    const parentNode = fs.getNode(parentDescriptor.inode, 1 /* Directory */);
    return new DirectoryFileDescriptor(deviceId, fd11, rights_base, rights_inheriting, fdflags11, fs.getOrCreateNode(parentNode, path, 1 /* Directory */, true).inode);
  }
  ...
  async function doGetFiletype(fileDescriptor, path) {
    const inode6 = fs.getNode(fileDescriptor.inode, 1 /* Directory */);
    try {
      const stat = await vscode_fs.stat(fs.getUri(inode6, path));
  ...
  const $this = {
    ...
    createStdioFileDescriptor(dirflags = Lookupflags.none, path, _oflags = Oflags.none, _fs_rights_base, fdflags11 = Fdflags.none, fd11) {
      if (path.length === 0) {
        throw new WasiError(Errno.inval);
      }
      ...
      return $this.path_open(parentDescriptor, dirflags, path, oflags8, fs_rights_base, getFileInheritingRights(readOnly), fdflags11, { next: () => fd11 });
    },
    ...
    async path_create_directory(fileDescriptor, path) {
      const inode6 = fs.getNode(fileDescriptor.inode, 1 /* Directory */);
      await vscode_fs.createDirectory(fs.getUri(inode6, path));
    },
    async path_filestat_get(fileDescriptor, _flags, path, result) {
      assertDirectoryDescriptor(fileDescriptor);
      const inode6 = fs.getNode(fileDescriptor.inode, 1 /* Directory */);
      const vStat = await vscode_fs.stat(fs.getUri(inode6, path));
      assignStat(result, fs.getOrCreateNode(inode6, path, vStat.type === import_vscode2.FileType.Directory ? 1 /* Directory */ : 0 /* File */, false).inode, vStat);
    },
    ...
    async path_open(parentDescriptor, _dirflags, path, oflags8, fs_rights_base, fs_rights_inheriting, fdflags11, fdProvider) {
      ...
      let filetype5 = await doGetFiletype(parentDescriptor, path);
      ...
      if (Oflags.creatOn(oflags8) && !entryExists) {
        ...
        const dirname2 = ral_default().path.dirname(path);
        if (dirname2 !== ".") {
          const dirFiletype = await doGetFiletype(parentDescriptor, dirname2);
      ...
      const result = filetype5 === Filetype.regular_file ? createFileDescriptor(parentDescriptor, fdProvider.next(), parentDescriptor.childFileRights(fs_rights_base), fdflags11, path) : createDirectoryDescriptor(parentDescriptor, fdProvider.next(), parentDescriptor.childDirectoryRights(fs_rights_base), fs_rights_inheriting | getDirectoryInheritingRights(readOnly), fdflags11, path);
    ...
    async path_remove_directory(fileDescriptor, path) {
      assertDirectoryDescriptor(fileDescriptor);
      const inode6 = fs.getNode(fileDescriptor.inode, 1 /* Directory */);
      const targetNode = fs.getNodeByPath(inode6, path, 1 /* Directory */);
      let filestat10;
      ...
      await vscode_fs.delete(fs.getUri(inode6, path), { recursive: false, useTrash: ral_default().workbench.hasTrash });
    ...
    async path_unlink_file(fileDescriptor, path) {
      assertDirectoryDescriptor(fileDescriptor);
      const inode6 = fs.getNode(fileDescriptor.inode, 1 /* Directory */);
      const targetNode = fs.getNodeByPath(inode6, path, 0 /* File */);
      let filestat10;
      let content;
      if (targetNode !== void 0 && targetNode.refs > 0) {
        try {
...
function create3(deviceId, baseUri, dump) {
  ...
  const $driver = {
    ...
    path_open(fileDescriptor, _dirflags, path, oflags8, fs_rights_base, fs_rights_inheriting, fdflags11, fdProvider) {
      assertDirectoryDescriptor(fileDescriptor);
      const target = $fs.findNode(fileDescriptor.node, path);
      if (target === void 0) {
        if (Oflags.creatOn(oflags8)) {
    ...
    path_readlink(fileDescriptor, path) {
      assertDirectoryDescriptor(fileDescriptor);
      const target = $fs.findNode(fileDescriptor.node, path);
      if (target === void 0) {
        throw new WasiError(Errno.noent);
...
var MemoryFileSystem = class extends BaseFileSystem {
  ...
  createDirectory(path) {
    const dirname2 = paths.dirname(path);
    const basename2 = paths.basename(path);
    const parent = this.getDirectoryNode(dirname2);
    const node = DirectoryNode3.create(parent, this.nextInode(), basename2, timeInNanoseconds(Date.now()));
    parent.entries.set(basename2, node);
  }
  createFile(path, content) {
    const dirname2 = paths.dirname(path);
    const basename2 = paths.basename(path);
    const parent = this.getDirectoryNode(dirname2);
    const node = FileNode3.create(parent, this.nextInode(), basename2, timeInNanoseconds(Date.now()), content);
    parent.entries.set(basename2, node);
  }
  createReadable(path) {
    const dirname2 = paths.dirname(path);
    const basename2 = paths.basename(path);
    const parent = this.getDirectoryNode(dirname2);
    const node = CharacterDeviceNode.create(parent, this.nextInode(), basename2, timeInNanoseconds(Date.now()), new ReadableStream(), void 0);
    parent.entries.set(basename2, node);
    return node.readable;
  }
  createWritable(path, encoding) {
    const dirname2 = paths.dirname(path);
    const basename2 = paths.basename(path);
    const parent = this.getDirectoryNode(dirname2);
    const node = CharacterDeviceNode.create(parent, this.nextInode(), basename2, timeInNanoseconds(Date.now()), void 0, new WritableStream(encoding));
    parent.entries.set(basename2, node);
    return node.writable;
  }
  getDirectoryNode(path) {
...
function create5(deviceId, rootFileDescriptors, mountPoints) {
  ...
  const $driver = {
    ...
    async path_filestat_get(fileDescriptor, flags, path, result) {
      assertDirectoryDescriptor(fileDescriptor);
    ...
    async path_open(parentDescriptor, dirflags, path, oflags8, fs_rights_base, fs_rights_inheriting, fdflags11, fdProvider) {
      assertDirectoryDescriptor(parentDescriptor);
...
((MapDirDescriptors2) => {
  ...
  function mapWorkspaceFolder(folder, single) {
    const path = ral_default().path;
    const mountPoint = single ? path.join(path.sep, "workspace") : path.join(path.sep, "workspaces", folder.name);
...
var FileSystems = class {
  ...
  async createExtensionLocationFileSystem(descriptor) {
    ...
    const paths2 = ral_default().path;
    const basename2 = paths2.basename(descriptor.path);
    const dirname2 = paths2.dirname(descriptor.path);
    const dirDumpFileUri = import_vscode7.Uri.joinPath(descriptor.extension.extensionUri, dirname2, `${basename2}.dir.json`);
    try {
        ...
        if (typeof arg === "string") {
          ...
          for (const [uri2, mountPoint] of uriToMountPoint) {
            if (arg_str.startsWith(uri2)) {
              args.push(path.join(mountPoint, arg_str.substring(uri2.length)));
              mapped = true;
              break;
            }
          }
...
// src/web/process.ts
var BrowserServiceConnection = class extends ServiceConnection {
  constructor(wasiService, port, logChannel) {
    super(wasiService, logChannel);
    __publicField(this, "port");
    this.port = port;
    this.port.onmessage = (event2) => {
      this.handleMessage(event2.data).catch((error) => ral_default().console.error(error));
    };
  }
  postMessage(message) {
    try {
...
var BrowserWasiProcess = class extends WasiProcess {
  ...
  async startMain(wasiService) {
    const filename = import_vscode12.Uri.joinPath(this.baseUri, "./dist/web/mainWorker.js").toString();
  ...
  async startThread(wasiService, tid2, start_arg) {
    ...
    const filename = import_vscode12.Uri.joinPath(this.baseUri, "./dist/web/threadWorker.js").toString();
