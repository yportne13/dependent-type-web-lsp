...
use cxt::Cxt;
use crate::parser::{syntax::{Either, Icit, Raw}, IError};
...
pub fn empty_span<T>(data: T) -> Span<T> {
    Span {
        data,
        start_offset: 0,
        end_offset: 0,
...
pub fn run(input: &str, path_id: u32) -> Result<String, Error> {
    ...
    let ast = crate::parser::parser(&preprocess(input), path_id).unwrap();
    let mut cxt = Cxt::new();
    ...
    for tm in ast.0 {
        match &tm {
            crate::parser::syntax::Decl::Def { name, .. }
            | crate::parser::syntax::Decl::Enum { name, .. }
            | crate::parser::syntax::Decl::TraitDecl { name, .. } => {
                println!("> {}", name.data);
                //cxt.print_env(&infer);
            },
            crate::parser::syntax::Decl::Println(raw) => {},
            crate::parser::syntax::Decl::ImplDecl { .. } => {
                println!("> impl");
            }
        }
