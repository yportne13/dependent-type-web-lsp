...
(() => {
    ...
    function create() {
      ...
      return {
        ...
        fd_prestat_dir_name: (_memory, result, fd3, pathPtr, pathLen) => {
          if (result === Errno.success) {
            const memory = new Memory(_memory);
            const path2 = memory.readString(pathPtr, pathLen);
            preStats.set(fd3, path2);
            fileDescriptors.set(fd3, path2);
            return `fd_prestat_dir_name(fd: ${fd3}) => [path: ${path2}, result: ${Errno.toString(result)}]`;
          } else {
        ...
        path_open: (_memory, result, fd3, dirflags, path_ptr, path_len, oflags3, _fs_rights_base, _fs_rights_inheriting, fdflags3, fd_ptr) => {
          const memory = new Memory(_memory);
          const path2 = memory.readString(path_ptr, path_len);
          if (result === Errno.success) {
            const resultFd = memory.readUint32(fd_ptr);
            const message = `path_open(fd: ${fd3} => ${getFileDescriptorPath(fd3)}, dirflags: ${Lookupflags.toString(dirflags)}, path: ${path2}, oflags: ${Oflags.toString(oflags3)}, fdflags: ${Fdflags.toString(fdflags3)}) => [fd: ${resultFd}, result: ${Errno.toString(result)}]`;
            if (result === Errno.success) {
              const parentPath = fileDescriptors.get(fd3);
              fileDescriptors.set(resultFd, parentPath !== void 0 ? ral_default().path.join(parentPath, path2) : path2);
            }
            return message;
          } else {
            return `path_open(fd: ${fd3} => ${getFileDescriptorPath(fd3)}, dirflags: ${Lookupflags.toString(dirflags)}, path: ${path2}, oflags: ${Oflags.toString(oflags3)}, fdflags: ${Fdflags.toString(fdflags3)}) => [result: ${Errno.toString(result)}]`;
          }
        },
  ...
  var ThreadNodeHostConnection = class extends NodeHostConnection {
    constructor(port) {
      super(port);
    ...
    async handleMessage(message) {
      if (StartThreadMessage.is(message)) {
        ...
        instance.exports.wasi_thread_start(message.tid, message.start_arg);
  ...
  main(import_worker_threads.parentPort).catch(ril_default().console.error).finally(() => {
    import_worker_threads.parentPort?.unref();
  });
})();
