...
use crate::parser::syntax::{Decl, Either, Icit, Raw, Pattern};
    ...
    pub fn check(&mut self, cxt: &Cxt, t: Raw, a: Val) -> Result<Tm, Error> {
        ...
        match (t, self.force(a)) {
            ...
            (Raw::Match(expr, clause), expected) => {
                ...
                let mut compiler = Compiler::new(expected, expr_span);
            ...
            Decl::Enum {
                ...
                let default_ret = params
                    ...
                    .fold(Raw::Var(name.clone()), |ret, x| {
                        Raw::App(Box::new(ret), Box::new(Raw::Var(x.0.clone())), Either::Icit(x.2))
                    });
                let new_cases = cases
                    .clone()
                    .into_iter()
