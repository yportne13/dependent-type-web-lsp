...
var MemoryFileSystem = class extends BaseFileSystem {
  ...
  createDirectory(path) {
    ...
    const node = DirectoryNode.create(parent, this.nextInode(), basename2, timeInNanoseconds(Date.now()));
  ...
  createFile(path, content) {
    ...
    const basename2 = paths.basename(path);
    const parent = this.getDirectoryNode(dirname2);
    const node = FileNode.create(parent, this.nextInode(), basename2, timeInNanoseconds(Date.now()), content);
    parent.entries.set(basename2, node);
  }
  createReadable(path) {
    const dirname2 = paths.dirname(path);
...
function create3(deviceId, baseUri, readOnly = false) {
  ...
  const $this = {
    ...
    async path_filestat_get(fileDescriptor, _flags, path, result) {
      ...
      assignStat(result, fs.getOrCreateNode(inode6, path, vStat.type === import_vscode5.FileType.Directory ? 1 /* Directory */ : 0 /* File */, false).inode, vStat);
    },
...
var FileSystems = class {
  ...
  async createExtensionLocationFileSystem(descriptor) {
    ...
    const dirDumpFileUri = import_vscode8.Uri.joinPath(descriptor.extension.extensionUri, dirname2, `${basename2}.dir.json`);
    try {
